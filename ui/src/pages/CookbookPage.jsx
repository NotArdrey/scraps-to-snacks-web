...
export default function CookbookPage() {
  ...
  const handleSaveEdit = (id) => {
    setRecipes(recipes.map(r => r.id === id ? { ...r, ...editForm } : r));
    setEditingId(null);
  };
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '3rem auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '3rem' }}>
        <div style={{ background: 'var(--surface-color)', padding: '1rem', borderRadius: '50%', backdropFilter: 'blur(10px)' }}>
          <Book size={32} color="var(--primary-color)" />
        </div>
        <div>
          <h2 className="page-title" style={{ textAlign: 'left', margin: 0 }}>My Cookbook</h2>
          <p className="page-subtitle" style={{ textAlign: 'left', margin: 0 }}>Your saved AI recipes and cooking history.</p>
        </div>
      </div>
        ...
        <div className="grid-container">
          {recipes.map(recipe => (
            <div key={recipe.id} className="glass-card" style={{ display: 'flex', flexDirection: 'column', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: '1rem', right: '1rem', display: 'flex', gap: '0.5rem' }}>
                {editingId === recipe.id ? (
                  <>
                    ...
                    <button onClick={() => handleSaveEdit(recipe.id)} className="btn-primary" style={{ padding: '0.4rem', borderRadius: '50%' }} title="Save">
                      <Save size={16} />
                    </button>
                  </>
                ) : (
              ...
              {/* Permanent View for Ingredients and Instructions */}
